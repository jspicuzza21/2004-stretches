...
describe('lockBox functionality', () => {
  xtest('lockbox should take two arguments -> code, message and fail if incorrect types', () => {
    expect(() => lockBox('123', 123)).toThrow();
  });
  xtest('lockBox should have methods -> modCode, modMessage, and revealMessage', () => {
  ...
  xtest('modCode should modify initial code supplied', () => {
    let safe = lockBox(123, 'secret message');
    expect(safe.modCode(123, 456)).toEqual(456);
  });
  xtest('modMessage should change initial message supplied', () => {
    let safe = lockBox(123, 'secret message');
    expect(safe.modMessage(123, 'new secret message')).toEqual(
      'new secret message'
    );
  });
  xtest('revealMessage should fail if incorrect code', () => {
    let safe = lockBox(123, 'secret message');
    expect(() => safe.revealMessage(456)).toThrow();
  });
  xtest('revealMessage should retieve secret message', () => {
    let safe = lockBox(123, 'secret message');
